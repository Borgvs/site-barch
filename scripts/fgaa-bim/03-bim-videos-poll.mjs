#!/usr/bin/env node
/**
 * FGAA-BIM v1.0 · Fase 3 polling — aguarda os 5 jobs Veo 3.1 BIM e baixa
 */
...
const VIDEOS_DIR = path.resolve("tmp/bim-v1/videos");
const jobs = JSON.parse(await fs.readFile(path.join(VIDEOS_DIR, "jobs.json"), "utf8"));

console.log(`\n━━━ Polling ${jobs.length} jobs Veo 3.1 BIM ━━━\n`);
...
async function pollJob(job) {
  ...
  try {
    ...
    if (s.status === "COMPLETED") {
      ...
      if (!url) return { ...job, error: `sem URL: ${JSON.stringify(result)}` };
